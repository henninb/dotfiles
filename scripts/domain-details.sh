...
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L -o /dev/null -v -X GET "https://$domain" > "/tmp/domain-$$.log" 2>&1
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L "https://$domain"  | grep -i "Content-Security-Policy" || echo "no CSP header response"
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L "https://$domain" | grep -i -w '^server:' || echo "no server header response"
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L "https://$domain" | grep -i -w '^cf-cache-status:' || echo "no cf-cache-status header response"
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L "https://$domain" | grep -i -w '^x-yottaa-os:' || echo "no x-yottaa-os header response"
curl --max-time 15 -H "User-Agent: $user_agent" --cookie-jar /tmp/mycookies.txt -s -I -L "https://$domain" | awk '/^HTTP/{print $1}'
