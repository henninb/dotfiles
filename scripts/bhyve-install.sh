...
if [ "$OS" = "FreeBSD" ]; then
  sudo mkdir -p /vm
  sudo mkdir -p /vm/iso
  doas pkg install -y vm-bhyve
  doas pkg install -y grub2-bhyve
  doas pkg install -y bhyve-rc
  doas pkg install -y bhyve-firmware
...
#  sudo vm switch add public em0
  doas vm switch add public vtnet0
  doas vm switch add public re0
  doas sysctl net.link.tap.up on open=1
